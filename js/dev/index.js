...
function initSliders() {
  if (document.querySelector(".gallery-mob__slider")) {
    new Swiper(".gallery-mob__slider", {
  ...
  function updateSlideCounter(swiper) {
    ...
    const counterEl = swiper.el.closest(".ba__slider").querySelector(".ba__counter");
    if (counterEl) {
  ...
  function updateSlideCounterReviews(swiper) {
    ...
    const counterEl = swiper.el.closest(".reviews__slider").querySelector(".reviews__counter");
    if (counterEl) {
  ...
  new Swiper(".reviews__slider", {
    ...
    breakpoints: {
      320: {
        slidesPerView: 1,
        slidesPerGroup: 1,
        // тоже для мобильной версии лучше явно указать
        spaceBetween: 20,
        autoHeight: true
      },
      768: {
        slidesPerView: 2,
        slidesPerGroup: 1,
        // и тут, чтобы повсюду листало по одному слайду
        spaceBetween: 20
      }
    },
    on: {
      init: function() {
        updateSlideCounterReviews(this);
      },
      slideChange: function() {
        updateSlideCounterReviews(this);
      }
...
class BeforeAfter {
  ...
  beforeAfterItemInit(beforeAfter) {
    ...
    parseFloat(window.getComputedStyle(beforeAfterArrow).getPropertyValue("width"));
    ...
    function beforeAfterArrowMove(e) {
      ...
      if (posLeft <= beforeAfterSizes.width && posLeft > 0) {
        ...
        beforeAfterArrow.style.cssText = `left:calc(${way}% - 32px)`;
        afterItem.style.cssText = `width: ${100 - way}%`;
      } else if (posLeft >= beforeAfterSizes.width) {
        beforeAfterArrow.style.cssText = `left: calc(100% - 32px)`;
...
document.addEventListener("DOMContentLoaded", function() {
  const steps = document.querySelectorAll(".test-quiz__step");
  const asks = document.querySelectorAll(".test-quiz__ask");
  let currentStep = 0;
  const nextButtons = document.querySelectorAll(".test-quiz__next");
  const prevButtons = document.querySelectorAll(".test-quiz__prev");
  function showStep(index) {
    steps.forEach((step, i) => {
      step.style.display = i === index ? "block" : "none";
    });
    asks.forEach((ask, i) => {
      ask.classList.toggle("active", i === index);
    });
  }
  showStep(currentStep);
  document.querySelectorAll(".test-quiz__row").forEach((row) => {
    row.addEventListener("change", (e) => {
      const parentStep = e.target.closest(".test-quiz__step");
      const nextBtn = parentStep.querySelector(".test-quiz__next");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  phoneInputs.forEach((input) => {
    ...
    input.addEventListener("click", onClick2);
...
function switchVerToHor() {
  if (window.innerWidth <= 991.98) {
    document.querySelectorAll(".option_ver").forEach((el) => {
      el.classList.remove("option_ver");
      el.classList.add("option_hor");
    });
    document.querySelectorAll(".row_ver").forEach((el) => {
      el.classList.remove("row_ver");
      el.classList.add("row_hor");
    });
    document.querySelectorAll(".question_ver").forEach((el) => {
      el.classList.remove("question_ver");
      el.classList.add("question_hor");
    });
  }
}
window.addEventListener("resize", switchVerToHor);
switchVerToHor();
document.addEventListener("DOMContentLoaded", () => {
...
document.addEventListener("DOMContentLoaded", () => {
  const form = document.querySelector(".test-quiz__questions");
  if (form) {
    form.addEventListener("submit", function(e) {
      e.preventDefault();
      setTimeout(() => {
